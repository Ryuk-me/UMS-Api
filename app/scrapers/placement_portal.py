    ...
    async with aiohttp.ClientSession() as session:
        ...
        user_details = {
            ...
            "basic_details": regex_replace_str(
                regex_replace_str(left_details_lis[2].find("span").get_text(strip=True))
            ),
        ...
        for tr in tpc_employee_details.find_all("tr")[1:]:
            tds = tr.find_all("td")
            mentor_name = regex_replace_str(tds[1].find("span").get_text(strip=True))
            try:
                ...
                mentor_email = None
            phone_number = regex_replace_str(tds[3].find("span").get_text(strip=True))
        ...
        upcoming_placemnet_drives = soup.find("table", class_="main_table main_table12")
        upcoming_drives = []
                ...
                upcoming_drives.append(
                    {
                        ...
                        "hall_ticket": requests.utils.requote_uri(
        ...
        total_drives_held_data = (
            divs[4]
            .next_sibling.next_sibling.find_next("table")
            .find("table")
            .find("table")
        )
        drives_count = []
        ...
        user_details["fines"] = []
        fine_details = (
            divs[4].next_sibling.next_sibling.find_next_sibling("div").find_all("table")
        )
        if len(fine_details) > 0:
            for tr in fine_details[-1].find_all("tr")[1:]:
                tds = tr.find_all("td")
